package com.luminousdust.logic;
...
public class DustSpawner {
    public static void tick (Level level, Player player, Minecraft mc) {
        int radius = LumDustConf.AMBIENT_RADIUS.get();
        int maxCap = LumDustConf.AMBIENT_BLOCK_CAP.get();
                ...
                for (int z = -radius; z <= radius; z++) {
                    if (Math.abs(x + y + z) % 40 == tickMod && x * x + y * y + z * z <= radiusSqr) {
                        ...
                        if (level.dimension() != Level.OVERWORLD || !isDay || !level.canSeeSky(mutablePos)) {
                            ...
                            int currentCount = com.luminousdust.client.particle.DustParticle.AMBIENT_COUNTS.get(posKey);
                            if (currentCount < maxCap) {
                                int blockLight = level.getBrightness(LightLayer.BLOCK, mutablePos);
                                if (blockLight < LumDustConf.MIN_BLOCK_LIGHT.get()) {
                                    ...
                                    if (!level.getFluidState(mutablePos).is(FluidTags.WATER) && !level.getFluidState(mutablePos).is(FluidTags.LAVA)) {
                                        ...
                                        if (state.getCollisionShape(level, mutablePos).isEmpty()) {
                                            ...
                                            if (currentCount < targetCap) {
                                                com.luminousdust.client.particle.DustParticle.PENDING_POS = mutablePos.immutable();
                                                int spawnCount = 1;
                                                if (currentCount == 0) {
                                                    spawnCount = 4;
        ...
        if (tick % 60L == 0L) {
            double hardCap = LumDustConf.AMBIENT_HARD_CAP.get();
        ...
        if (player.swingTime == 1 && mc.hitResult != null && mc.hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (level.getBlockState(breakPos).isAir()) {
                int count = LumDustConf.BREAK_PARTICLE_COUNT.get();
                double speed = LumDustConf.BREAK_PARTICLE_SPEED.get();
